...
export const projectPopupData = [
  {
    ...
    retrospective: `이번 프로젝트를 통해 기능 구현뿐만 아니라 사용자 흐름과 상태 관리의 일관성이 UX에 큰 영향을 준다는 점을 경험했습니다. 특히 URL 상태 동기화를 통해 새로고침이나 페이지 이동에도 사용자 맥락을 유지하는 설계의 중요성을 체감했으며, React 훅을 활용한 상태 관리 역량을 강화할 수 있었습니다.

    상태 범위가 커지면서 전역 상태 관리 도구를 미처 도입하지 못해 프로젝트 후반부에 전체적인 상태 관리 로직이 다소 복잡해졌으나,리팩토링을 통해 구조를 정리하고 유지보수성을 개선했습니다.`,
  },
