...
export const projectPopupData = [
  {
    text: "ReadPick",
    person: "2명",
    date: "2025.03 ~ 2025.07 (리팩토링: 26.07)",
    rate: "프론트엔드 전담 (UX 설계 및 기능 구현)",
    purpose: `도서 탐색의 편의성과 감상평 기록까지의 과정을 간소화하여, 사용자가 불필요한 단계 없이 독서 경험을 기록하고 컨텐츠에 좀 더 집중할 수 있는 웹 서비스를 기획했습니다.`,
    info: `도서 탐색 과정에서 사용자가 겪는 피로도를 줄이고 좀 더 쉽게 접근하도록 도서명·작가명 기반의 직관적인 검색 기능을 구현했습니다. 또한 마이페이지에서 내가 쓴 리뷰와 찜목록을 바로 확인 가능하도록 설계하여 접근성을 높였습니다.
    
    또한 페이지 이동이나 새로고침 이후에도 검색 조건과 목록 상태가 유지되도록 URL 상태 동기화를 적용하여 사용자의 재탐색 부담을 줄이도록 했습니다.`,
    function: [
      "도서명·작가명 기반 도서 검색 기능",
      "도서별 감상평 및 리뷰 작성·수정·삭제 기능",
      "추천 도서 찜(관심 도서) 추가·제거 기능",
    ...
    step: [
...

      {
        ...
        src: projectPopup2,
        info: "도서 상세 화면에서 바로 감상평 또는 리뷰를 작성할 수 있도록 설계하여, 사용자가 바로바로 기록할 수 있습니다.",
      },
    ...
    technology: [
      {
        title: "상태 관리",
        info: "React Query를 활용한 상태 관리 및 비동기 데이터 최적화",
      },
      {
        title: "인증 처리",
        info: "Zustand와 persist 미들웨어를 활용한 토큰 기반 인증 상태를 관리하고, 인증이 필요한 기능에 대한 \n접근 제어",
      },
      {
        title: "API 관리",
      ...
      {
        title: "조건부 렌더링",
        info: "로그인 상태에 따라 리뷰 작성, 찜 기능 접근을 제어하여 사용자 시나리오 분기 처리",
      },
      {
        title: "UI 피드백 처리",
    ...
    refactoring: [
      {
        title: "문제 & 해결",
        info: `useState만으로 컴포넌트 내부 상태를 관리하려니 로직이 점차 복잡해지는 문제가 생겨 서버 데이터 관리와 비동기 처리를 React Query로 수정하였습니다.\n
        기존 UI 구조에서 중복 렌더링이 많아 유지보수가 어려웠고, 공통 컴포넌트로 분리해 재사용 가능한 구조로 리팩토링을 진행하였습니다.`,
      },
      {
        title: "결과",
        info: `API 요청을 최적화하고 코드가 명확하게 분리되어 변경 범위를 줄일수 있었고, 코드 유지보수성과 확장성이 크게 향상되었습니다.
        `,
      },
    ],

    retrospective: `이번 프로젝트는 단순히 기능을 구현하는 것을 넘어, 사용자 흐름과 상태 관리의 일관성이 UX에 큰 영향을 준다는 점을 경험했습니다.

    초기 개발 단계에서는 useState 위주로 진행하면서 비동기 상태의 범위에 따라 전체적인 상태 관리 로직이 복잡해지는 한계를 마주했습니다.
    이를 개선하기 위해 서버 상태는 React Query, 인증이나 세션 상태는 Zustand로 역할을 분담하면서 리팩토링을 진행했습니다. 이 과정을 통해 유지보수성이 높고 확장 가능한 프론트엔드 아키텍처를 설계하는 역량을 한 단계 성장시킬 수 있었습니다.
    `,
  },

  {
    ...
    React의 state를 활용하여 화면 상태를 관리하고, Swiper 라이브러리를 적용해 제주도의 대표 음식 정보를 슬라이드 형태로 탐색할 수 있도록 구현했습니다.
    ...
    step: [
      {
        ...
        src: projectPopup4,
        info: "제주 관광 포털 메인 화면에서 여행도우미 퀵 내비게이션, 추천 여행코스, 관광 지도를 한눈에 확인할 수 있습니다.",
      },
    ...
    technology: [
      {
        title: "API 관리",
        info: "MSW를 활용하여 관광지, 축제, 음식 데이터 API 호출 환경을 구성하고 요청/응답 구조를 모킹",
      },
      {
        title: "UI 인터랙션",
...

  {
    ...
    rate: "100%",
    purpose:
      "서울숲에서 개최된 <내 이름은 빨강머리 앤> 전시회를 찾는 관람객들이 전시 관련된 간단한 정보와 위치, 프로그램 등의 정보를 얻을 수 있도록 돕는 웹 서비스를 기획했습니다. ",
    info: `전시의 메인 테마와 각 챕터별 전시작품들, 그리고 전시장 위치 정보를 직관적으로 탐색할 수 있도록 구성했습니다. 
    
    데이터 관리 측면에서는 Supabase를 연동하여 실제 서비스와 동일한 데이터 흐름을 구축하도록 했으며, Vercel을 통해 최적화된 배포 환경을 마련하여 사용자에게 빠르고 안정적인 웹 전시 환경을 제공합니다.`,

    function: [
      "React State를 활용한 동적 UI 제어",
    ...
    step: [
      {
        title: "STEP 01. 메인 페이지",
        src: projectPopup7,
        info: "메인 화면에서는 전시소개와 순서, 프로그램과 일러스트 작품 모음전을 간단하게 확인할 수 있습니다.",
      },

      {
        title: "STEP 02. Exhibition",
        src: projectPopup8,
        info: "챕터별 전시회 내용과 다양한 작품들을 한눈에 확인할 수 있습니다.",
      },
    ...
    technology: [
      ...
      {
        title: "배포 관리",
        info: "Vercel을 활용해 자동 배포 파이프라인을 구축하고, Supabase API Key 정보를 환경 변수로 관리",
      },
    ],
    ...
    영향을 주는 요소들을 경험할 수 있었습니다. 단순히 UI 구현을 넘어, 실제 API 호출 환경을 구성하고 최적화된
